...
function registerClientMethods(chatHub) {
    ...
    chatHub.client.onConnected = function (id, userName, allUsers, messages, userid) {
        ...
        for (i = 0; i < allUsers.length; i++) {
            ...
            AddUser(chatHub, allUsers[i].ConnectionId, allUsers[i].UserName, allUsers[i].Id);
        }
...
function AddUser(chatHub, id, name, userid) {
    ...
    if (connectionid != id) {
        if ($('#' + userid).length == 0) {
            code = $('<a id="' + userid + '" class="col-sm-12 bg-success" > <i class=\"fa fa-user\"></i> ' + name + '<a>');
        ...
        if ($('#curruser_' + userid).length == 0) {
            code = $('<div id="curruser_' + userid + '" class="col-sm-12 bg-info"  ><i class=\"fa fa-user\"></i> ' + name + '<div>');
...
function OpenPrivateChatWindow(chatHub, userid, userName) {
    var ctrId = 'private_' + userid;
    if ($('#' + ctrId).length > 0) return;
    createPrivateChatWindow(chatHub, userid, ctrId, userName, userName);
}
...
function createPrivateChatWindow(chatHub, userId, ctrId, userName, chatTitle) {
    ...
    $('#' + ctrId).siblings().append("<div id=\"typing_" + userId + "\" style=\"width:20px; height:20px; display:none; position:absolute; right:14px; top:8px\"><img height=\"20\" src=\"" + srp + "images/tenor.gif\" /></div>");
    ...
    var FromUserID = $("[id$=hdnCurrentUserID]").val();
    var ToUserID = userId;
